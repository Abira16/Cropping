...
def main():
    ...
    image_dir = os.path.join(os.getcwd(), 'test_data', model_name + '_results/*.png')
...
def process_image_named(name, threshold_cutoff = 0.90, use_transparency = False):
    ...
    result_img = io.imread(os.path.join(os.getcwd(), 'test_data', 'u2net' + '_results/'+name+'.png'))
    # convert result-image to numpy array and rescale(255 for RBG images)
    RESCALE = 255
    out_img = np.array(result_img)
    out_img =np.divide(out_img, RESCALE)
    ...
    if use_transparency:
        ...
        mask_img = Image.fromarray((rgba_out*RESCALE).astype('uint8'), 'RGBA')
    else:
        mask_img = Image.fromarray((out_img*RESCALE).astype('uint8'), 'RGB')

    # load and convert input to numpy array and rescale(255 for RBG images)
    input = io.imread(os.path.join(os.getcwd(), 'test_data/test_images/'+name+'.jpg'))
    inp_img = np.array(input)
    inp_img = np.divide(inp_img,RESCALE) 
    ...
    if use_transparency:
        ...
        rem_back_scaled = Image.fromarray((rem_back*RESCALE).astype('uint8'), 'RGB')
    ...
    startx = min(x_starts)+30
    endx = max(x_ends)-30
    starty = min(y_starts)+30
    endy = max(y_ends)-30
    ...
    #print(rem_back_scaled.size,"size",name)
    if use_transparency:
        cropped_rem_back_scaled.save(os.path.join(os.getcwd(),'test_data/cropped_result/'+name+'_cropped_no-bg.png'))
    else:
        cropped_rem_back_scaled.save(os.path.join(os.getcwd(),'test_data/cropped_result/'+name+'_cropped_no-bg.jpg'))

    cropped_mask_img = mask_img.crop((startx,starty,endx,endy))

    if use_transparency:
        cropped_mask_img.save(os.path.join(os.getcwd(),'test_data/cropped_result/crop mask/'+name+'_cropped_no-bg_mask.png'))
    else:
        cropped_mask_img.save(os.path.join(os.getcwd(),'test_data/cropped_result/crop mask/'+name+'_cropped_no-bg_mask.jpg'))
